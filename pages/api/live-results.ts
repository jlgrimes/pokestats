...
const fetchPlayerProfiles = async () => {
  ...
  console.log('fetchPlayerProfiles:', (performance.now() - perfStart) / 1000, 'sec');
  return profiles;
};
...
const uploadMissingPlayerProfiles = async (
  ...
  console.log('uploadMissingPlayerProfiles:', (performance.now() - perfStart) / 1000, 'sec');
};
...
const getPlayerDeckObjects = async (
  ...
  console.log('getPlayerDeckObjects:', (performance.now() - perfStart) / 1000, 'sec');
...
function mapResultsArray(
  ...
  console.log('mapResultsArray:', (performance.now() - perfStart) / 1000, 'sec');
...
export default async function handler(
  ...
  try {
    ...
    let parsedData = await getPokedata(req.query.id as string);
